...
static void print_help() {
	...
	printf("  -o <online>            Specify isp mode of camera_config_t\n");
...
int main(int argc, char** argv) {
	...
	while((c = getopt_long(argc, argv, "s:o:t:m:h",
...
static int create_camera_node(pipe_contex_t *pipe_contex) {
	...
	if (sensor_mode >= NORMAL_M && sensor_mode < INVALID_MOD) {
		...
		// sensor_config->vin_node_attr->lpwm_attr.enable = 1; // TODO: 增加Master模式
	}

	{
		...
		camera_config->addr = csi_config->sensor_addr;
	}
...
static int create_isp_node(pipe_contex_t *pipe_contex) {
...

	{
	 	...
	 	*/
		vp_csi_config_t *csi_config = &pipe_contex->csi_config;
		ret = vp_get_isp_channel_config_for_single_pipeline(
