	...
	vp_sensor_config_t *dummy_sensor_config)
{
	...
	*/
	...
	camera_config->gpio_enable = 0;
	camera_config->gpio_level = 0;
...
int main(int argc, char** argv) {
	...
	if (index < vp_get_sensors_list_number() && index >= 0) {
		...
		ret = vp_sensor_multi_fixed_mipi_host(vin_isp_contex.sensor_config,
				&used_mipi_host, &vin_isp_contex.csi_config);
		if (ret != 0) {
