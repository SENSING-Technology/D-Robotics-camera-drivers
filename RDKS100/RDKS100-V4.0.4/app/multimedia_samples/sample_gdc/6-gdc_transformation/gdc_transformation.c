...
int main(int argc, char** argv) {
	...
	while((c = getopt_long(argc, argv, "i:x:y:w:h",
...
int create_and_run_vflow(gdc_info_s *gdc_info,
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		ret = gdc_cfg_bin_gen(json_path, NULL, (void**)&cfg_buf, &config_size);
		if (ret != 0) {
			printf("gdc_cfg_bin_gen failed, ret = %d\n", ret);
			continue;
		}
		...
		hb_mem_flush_buf(gdc_info->bin_buf.fd, offset, config_size);

		ret = hbn_vnode_open(HB_GDC, hw_id, AUTO_ALLOC_ID, &gdc_vnode_fd);
		ERR_CON_EQ(ret, 0);

		ret = hbn_vnode_set_attr(gdc_vnode_fd, &gdc_setting);
		ERR_CON_EQ(ret, 0);

		ret = hbn_vnode_set_ichn_attr(gdc_vnode_fd, chn_id, &gdc_setting);
		ERR_CON_EQ(ret, 0);

		ret = hbn_vnode_set_ochn_attr(gdc_vnode_fd, chn_id, &gdc_setting);
		ERR_CON_EQ(ret, 0);
