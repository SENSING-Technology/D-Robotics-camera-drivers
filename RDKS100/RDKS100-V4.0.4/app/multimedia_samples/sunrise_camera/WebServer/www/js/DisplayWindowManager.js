	...
	init(mediaServerIPAddr, callbacks = {}) {
		...
		for (var i = 0; i < 16; i++) {
			...
			for (var j = 0; j < layoutCount; j++) {// 循环生成视频容器
				...
				var capturePymButton = this.createCaptureButton("PYM", "PYM 处理的 YUV 图", i + 1, j + 1);

				// 将按钮添加到按钮容器中
				captureButtonContainer.appendChild(captureRawButton);
				captureButtonContainer.appendChild(captureIspButton);
				captureButtonContainer.appendChild(capturePymButton);
...
When destroying a Display Window, if a player is in the startup state, stop it.");
		...
		for (let i = 1; i <= displayWindowCount; i++) {
			...
			const capturePymButton = document.getElementById(`capture_pym_${displayWindowCount}_${i}`);

			if (radioButton.checked) {
				if (captureRawButton) captureRawButton.style.display = 'flex';
				if (captureIspButton) captureIspButton.style.display = 'none';
				if (capturePymButton) capturePymButton.style.display = 'flex';
			} else {
				if (captureRawButton) captureRawButton.style.display = 'none';
				if (captureIspButton) captureIspButton.style.display = 'none';
				if (capturePymButton) capturePymButton.style.display = 'none'; //盒子模式有PYM, 由于回灌模式获取图片需要考虑线程同步，暂时关闭
			}
		}
	}
		...
		if (target.classList.contains("capture-button")) {
			...
			switch (true) {
				...
				case buttonId.startsWith("capture_isp"):
					...
					break;
				case buttonId.startsWith("capture_pym"):
					var layoutNum = buttonId.split("_")[2];
					var videoNum = buttonId.split("_")[3];

					if (this.userCallbacks && typeof this.userCallbacks.onCapturePYM === 'function') {
						this.userCallbacks.onCapturePYM(videoNum);
					} else {
						console.error("onCapturePYM is not defined or not a function!", this.userCallbacks);
					}

					console.log("Clicked PYM capture button for layout " + layoutNum + ", video " + videoNum);
					break;
				default:
					break;
			}
