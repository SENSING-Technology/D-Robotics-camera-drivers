	...
	std::vector<Detection> &dets)
{
	...
	*/
	...
	for (int i = 0; i < 5; i++)
	{
		auto *cls_data = reinterpret_cast<int32_t *>(tensors[i].sysMem.virAddr);
		float *de_cls = tensors[i].properties.scale.scaleData;

		auto *bbox_data =
			reinterpret_cast<int32_t *>(tensors[i + 5].sysMem.virAddr);
		float *de_bbox = tensors[i + 5].properties.scale.scaleData;

		auto *ce_data =
			reinterpret_cast<int32_t *>(tensors[i + 10].sysMem.virAddr);
		float *de_ce = tensors[i + 10].properties.scale.scaleData;

		int *shape = tensors[i].properties.validShape.dimensionSize;
