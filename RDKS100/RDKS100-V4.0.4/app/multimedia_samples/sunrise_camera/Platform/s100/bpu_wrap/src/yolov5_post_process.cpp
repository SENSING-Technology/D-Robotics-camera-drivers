...
void Yolov5doProcess(hbDNNTensor *tensor, Yolov5PostProcessInfo_t *post_info, int layer, std::vector<Detection> &dets) {
	...
	*/
	...
	auto *data = reinterpret_cast<int32_t *>(tensor->sysMem.virAddr);
...
char* Yolov5PostProcess(Yolov5PostProcessInfo_t *post_info) {
	...
	for (i = 0; i < default_yolov5_config.strides.size(); i++) {
		Yolov5doProcess(&tensor[i], post_info, i, dets);
	}
