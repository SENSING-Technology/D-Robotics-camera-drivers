...
static int vp_ion_camera_service_calculator(vp_ion_camera_service_param_t *param){
	...
	int pym_buffer_size = 0;
	if(param->is_enable_pym){
		pym_buffer_size = 0x8000;
	}

	printf("camera service ion : [mcm_buf_size:%d] [for_3dnr_buffer_size:%d] [isp_buffer_size:%d] [pym_buffer_size:%d]\n",
		mcm_buf_size, for_3dnr_buffer_size, isp_buffer_size, pym_buffer_size);
	return (mcm_buf_size + for_3dnr_buffer_size + isp_buffer_size + pym_buffer_size);
}
...
static int vp_ion_vflow_calculator(vp_ion_pipeline_param_t *param){
	...
	int pym_buffer_size = 0;
	int pym_buffer_single_size = 0;
	for(int i = 0; i < param->pym_valid_count; i++){
		vp_ion_buffer_param_t *pym = &param->pym[i];
		pym_buffer_single_size = ALIGN_4K(pym->width * pym->height * 3 / 2) + 4096;;
		pym_buffer_size += pym_buffer_single_size * pym->count;
	}
	printf("vflow ion : [gdc_bin_file_size:%d] [gdc_buffer_size:%d] [vin_buffer_size:%d] [isp_buffer_size:%d] [pym_buffer_size:%d]\n",
		gdc_bin_file_size, gdc_buffer_size, vin_buffer_size, isp_buffer_size, pym_buffer_size);

	return gdc_bin_file_size + gdc_buffer_size + vin_buffer_size + isp_buffer_size + pym_buffer_size;
}
...
static int vp_ion_camera_service_extern_calculator(vp_ion_camera_service_extern_param_t *param){
	...
	int pym_buffer_size = 0;
	if(param->is_used_pym){
		pym_buffer_size = 0x1000 + 0x1000;
	}

	int camera_service_fixed = 0xAF1000;
	return (isp_buffer_size + pym_buffer_size + camera_service_fixed);
}
