...
void *vp_allocate_image_frame(ImageFrame *image_frame)
{
	...
	image_frame->hbn_vnode_image_group = (hbn_vnode_image_group_t *)malloc(sizeof(hbn_vnode_image_group_t));
	if (image_frame->hbn_vnode_image_group == NULL) {
		SC_LOGE("malloc hbn_vnode_image_group failed.\n");
		return NULL;
	}
...
void vp_free_image_frame(ImageFrame *image_frame)
{
	...
	if (image_frame->hbn_vnode_image_group != NULL) {
		free(image_frame->hbn_vnode_image_group);
		image_frame->hbn_vnode_image_group = NULL;
	}
}
...
{
	const char* soc_name_str = "/sys/class/boardinfo/board_name";
	FILE *file = fopen(soc_name_str, "r");
	if (file == NULL) {
		SC_LOGE("Failed to open %s", soc_name_str);
		return -1;
	}

	if (fgets(chip_type, 16, file) == NULL) {
		SC_LOGE("Failed to read from %s", soc_name_str);
		fclose(file);
		return -1;
	}
...
int vp_flow_get_frame(vp_vflow_contex_t *vp_vflow_contex, ImageFrame *frame){
	int ret = 0;
	if(vp_vflow_contex->gdc_info.status == GDC_STATUS_OPEN){
		ret = vp_gdc_get_frame(vp_vflow_contex, frame);
	}else{
		ret = vp_pym_get_frame(vp_vflow_contex, frame);
	}
	return ret;
}

int vp_flow_release_frame(vp_vflow_contex_t *vp_vflow_contex, ImageFrame *frame){
	int ret = 0;
	if(vp_vflow_contex->gdc_info.status == GDC_STATUS_OPEN){
		ret = vp_gdc_release_frame(vp_vflow_contex, frame);
	}else{
		ret = vp_pym_release_frame(vp_vflow_contex, frame);
	}
	return ret;
}