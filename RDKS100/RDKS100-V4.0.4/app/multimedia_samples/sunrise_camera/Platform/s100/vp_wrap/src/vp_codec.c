...
int32_t vp_codec_encoder_set_input(media_codec_context_t *context, ImageFrame *pym_frame){

	if ((context == NULL) || (pym_frame == NULL)
		|| (pym_frame->hbn_vnode_image == NULL)){
		SC_LOGE("codec param is NULL!\n");
		return -1;
	}
	int32_t ret = 0;
	media_codec_buffer_t buffer;
	hbn_vnode_image_t *hbn_vnode_image = pym_frame->hbn_vnode_image;
...
int32_t vp_codec_set_input(media_codec_context_t *context, ImageFrame *frame, int32_t eos)
{
	...
	if (context->encoder == true)
	{
		...
		if(context->video_enc_params.external_frame_buf){
			...
			if( (buffer->vframe_buf.size != 0) && (frame->plane_count == 1)) {
				memcpy(buffer->vframe_buf.vir_ptr[0], frame->data[0], buffer->vframe_buf.size);
			} else {
				uint8_t *dst_ptr = buffer->vframe_buf.vir_ptr[0];
				for(int i = 0; i< frame->plane_count; i++){
					memcpy(dst_ptr, frame->data[i], frame->data_size[i]);
					dst_ptr += frame->data_size[i];
				}
			}
			buffer->vframe_buf.size = 0;
			for(int i = 0; i< frame->plane_count; i++){
				buffer->vframe_buf.size += frame->data_size[i];
			}
		}
	}
	else
...
#if 0
			printf("[Decoder] width %d height %d stride %d vstride %d\n",
				buffer->vframe_buf.width, buffer->vframe_buf.height,
				buffer->vframe_buf.stride, buffer->vframe_buf.vstride);
			printf("[Decoder] frame->data_size[0] %d  %p frame->data_size[1] %d %p\n",
				frame->data_size[0], frame->data[0], frame->data_size[1], frame->data[1]);
#endif
