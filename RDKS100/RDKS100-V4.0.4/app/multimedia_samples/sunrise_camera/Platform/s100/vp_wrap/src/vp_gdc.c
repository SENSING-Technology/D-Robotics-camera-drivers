...
static gdc_list_info_t g_gdc_list_info[] = {
	{
		.sensor_name = "imx219",
		.gdc_file_name = "../gdc_bin/imx219_gdc.bin",
		.is_valid = -1
	}
};
...
int32_t vp_gdc_init(vp_vflow_contex_t *vp_vflow_contex){
	...
	gdc_setting.gdc_config.config_addr = vp_vflow_contex->gdc_info.bin_buf.phys_addr;
	gdc_setting.gdc_config.config_size = vp_vflow_contex->gdc_info.bin_buf.size;
	gdc_setting.binary_ion_id = vp_vflow_contex->gdc_info.bin_buf.share_id;
	gdc_setting.binary_offset = vp_vflow_contex->gdc_info.bin_buf.offset;
	...
	int input_width = vp_vflow_contex->gdc_info.input_width;
	int input_height = vp_vflow_contex->gdc_info.input_height;
	...
	ret = hbn_vnode_set_attr(vp_vflow_contex->gdc_info.gdc_fd, &gdc_setting);
	if(ret != 0){
	...
	uint32_t chn_id = 0;
	ret = hbn_vnode_set_ichn_attr(vp_vflow_contex->gdc_info.gdc_fd, chn_id, &gdc_setting);
	if(ret != 0){
	...
	ret = hbn_vnode_set_ochn_attr(vp_vflow_contex->gdc_info.gdc_fd, chn_id, &gdc_setting);
	if(ret != 0){
...
int32_t vp_gdc_get_frame(vp_vflow_contex_t *vp_vflow_contex, ImageFrame *frame)
{
	int32_t ret = 0;
	int ochn_id = 0;
	hbn_vnode_handle_t gdc_handle = vp_vflow_contex->gdc_info.gdc_fd;
...
int32_t vp_gdc_release_frame(vp_vflow_contex_t *vp_vflow_contex, ImageFrame *frame)
{
	int32_t ret = 0;
	int ochn_id = 0;
	hbn_vnode_handle_t gdc_handle = vp_vflow_contex->gdc_info.gdc_fd;
