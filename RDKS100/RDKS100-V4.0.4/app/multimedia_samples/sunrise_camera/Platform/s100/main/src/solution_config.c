...
static key_info_t csi_info_key[] = {
	...
	MAKE_KEY_INFO(csi_info_t, KEY_TYPE_S32, sensor_addr, NULL),
...
static key_info_t cfg_cam_vpp_key[] = {
	...
	MAKE_KEY_INFO(solution_cfg_cam_vpp_t, KEY_TYPE_S32, sensor_addr, NULL),
...
void print_solution_cfg(const solution_cfg_t *config)
{
	...
	for(int i = 0; i< csi_list_info->max_count; i++){
		printf("  	CSI_%d: [%s] [0x%02x]", csi_list_info->csi_info[i].index,
			csi_list_info->csi_info[i].sensor_config_list, csi_list_info->csi_info[i].sensor_addr);
	}
	printf("\n");
	...
	for (int i = 0; i < config->cam_solution.max_pipeline_count; i++)
	{
		...
		printf("    Sensor Addr: %02x\n", config->cam_solution.cam_vpp[i].sensor_addr);
...
int32_t solution_cfg_update_camera_config(){
	...
	for(int i = 0; i < csi_list_info->max_count; i++){
		...
		cam_vpp->sensor_addr = csi_list_info->csi_info[i].sensor_addr;
		cam_vpp->is_valid = 1;
