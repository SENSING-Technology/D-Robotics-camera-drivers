...
void vpp_codec_buf_to_bpu_buffer_info(const media_codec_buffer_t *src, bpu_buffer_info_t *dst)
{
	if (!src || !dst) return;
	int plane_count = 2; //编码器输出的Plane 固定为2

	dst->plane_cnt = plane_count;
	dst->width = src->vframe_buf.width;
	dst->height = src->vframe_buf.height;
	dst->w_stride = src->vframe_buf.stride;

	for (int i = 0; i < plane_count; ++i) {
		dst->addr[i] = src->vframe_buf.vir_ptr[i];
		dst->paddr[i] = src->vframe_buf.phy_ptr[i];
	}

	dst->tv.tv_sec = src->vframe_buf.pts / 1000000000;
	dst->tv.tv_usec = (src->vframe_buf.pts % 1000000000) / 1000;
...
void vpp_graphic_buf_to_bpu_buffer_info(const hbn_vnode_image_t *src, bpu_buffer_info_t *dst)
{
	...
	// printf("graphic_buf->plane_cnt: %d\n", graphic_buf->plane_cnt);
	...
	hbn_vnode_image_t *dst)
{
	...
	dst->buffer.format = MEM_PIX_FMT_NV12; // src->pix_fmt;
