...
static void print_help(void) {
	...
	printf("\t\tlink     --  Sensor link port number, serdes sensor must be configured according to the hardware connection, can be set to [0-3] 0:A 1:B 2:C 3:D.\n");
...
void *encode_pipe_data(void *context)
{
	...
	while (running) {
        ...
        if (elapsed >= 1.0) {
            ...
            printf("Pipeline %d FPS: %u\n", pipeline_info->pipeline_index, fps);
            // 重置计数器
            frame_count = 0;
