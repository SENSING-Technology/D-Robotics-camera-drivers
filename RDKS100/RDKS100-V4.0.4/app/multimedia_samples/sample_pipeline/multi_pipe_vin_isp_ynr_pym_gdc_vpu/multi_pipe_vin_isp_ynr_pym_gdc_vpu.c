...
#define CODEC_MIN_ENC_PIX_WIDTH              256
#define CODEC_MIN_ENC_PIX_HEIGHT             128
...
static void print_help(void) {
	...
	printf("Options:\n");
	printf("-c, --config=\"sensor=id link=port channel=pym_chn type=TYPE output=FILE, 'channel' and 'type' and 'output' is not mandatory\"\n");
	...
	printf("\t\tlink     --  Specify the port for connecting serdes sensors, 0:A 1:B 2:C 3:D, can be set to [0-3].\n");
...
static int create_encodec(pipeline_info_t *pipe_info, media_codec_context_t *media_context)
{
	...
	if(encode_width < CODEC_MIN_ENC_PIX_WIDTH) {
		printf("\n[Warning] encoder support min width is %d, but pym channel [%d] output is %d, so select other channel.\n",
			CODEC_MIN_ENC_PIX_WIDTH, pipe_info->pym_select_chn, encode_width);
		return -1;
	}
	if(encode_height < CODEC_MIN_ENC_PIX_HEIGHT) {
		printf("\n[Warning] encoder support min height is %d, but pym channel [%d] output is %d, so select other channel.\n",
			CODEC_MIN_ENC_PIX_HEIGHT, pipe_info->pym_select_chn, encode_height);
		return -1;
	}
	if(strlen(pipe_info->output_file) == 0){
		sprintf(pipe_info->output_file, "pipeline%d_%dx%d_%dfps.%s",
			pipe_info->pipeline_index, encode_width, encode_height,
			pipe_info->pipe_contexts.sensor_config->camera_config->fps,
			pipe_info->encode_type);
	}

	printf("[%d] encoder input resolution is %d*%d, output file is %s.\n",
		pipe_info->pipeline_index, encode_width, encode_height, pipe_info->output_file);
...
int create_pym_node(pipe_contex_t *pipe_contex, int hw_id, int slot_id, int pym_mode, int pym_select_chn) {
	...
	pym_cfg.layer_num_trans_next = pym_select_chn;
	...
	for(int i = 0; i < MAX_DS_NUM; i++) {
		...
		if(bl_width < PYM_MIN_WIDTH) {
			printf("[Warnning] ochn[%d] ratio= %d, width = %d < PYM_MIN_WIDTH(%d), so not enable.\n",
				i, ratio, bl_width, PYM_MIN_WIDTH);
			continue;
		}
		if(bl_height < PYM_MIN_HEIGHT) {
			printf("[Warnning] ochn[%d] ratio= %d, height = %d < PYM_MIN_HEIGHT(%d), so not enable.\n",
...
#else //编码器的限制：PYM 输出给编码器
		//限制3: 本程序中 PYM的输出和会交给编解码，编解码的输入需要 8字节对齐
		roi_box->out_width = FLOOR_ALIGN_8(roi_box->region_width);
		roi_box->out_height = FLOOR_ALIGN_8(roi_box->region_height);
#endif
		...
		node_out_info->wstride = roi_box->wstride_uv;
...
static int32_t read_gdc_config(const char *gdc_bin_file, hb_mem_common_buf_t *bin_buf)
{
...
int create_gdc_vnode(pipe_contex_t *pipe_contex, const char *gdc_bin_file, int input_width, int input_height) {
	int ret = 0;
	uint32_t hw_id = 0;
	uint32_t chn_id = 0;
	...
	ret = read_gdc_config(gdc_bin_file, &pipe_contex->bin_buf);
	if(ret != 0) {
	...
	gdc_setting.gdc_config.config_addr = pipe_contex->bin_buf.phys_addr;
	gdc_setting.gdc_config.config_size = pipe_contex->bin_buf.size;
	...
	gdc_setting.binary_ion_id = pipe_contex->bin_buf.share_id;
	gdc_setting.binary_offset = pipe_contex->bin_buf.offset;
...
const char *get_gdc_bin_file_path(const char *sensor_name, int width, int height);
...
static int create_vflow(pipeline_info_t *pipeline_info)
{
	...
	// 1. 创建 node
	ret = create_camera_node(pipe_contex, pipeline_info->link_port);
	ERR_CON_EQ(ret, 0);
	...
	//for gdc
	int input_width = pipe_contex->pym_out_info[pipeline_info->pym_select_chn].width;
	int input_height = pipe_contex->pym_out_info[pipeline_info->pym_select_chn].height;
	char *sensor_name = pipeline_info->pipe_contexts.sensor_config->sensor_name;
	pipeline_info->gdc_bin_file = get_gdc_bin_file_path(sensor_name, input_width, input_height);
	if(pipeline_info->gdc_bin_file) {
		printf("	[%d] use [gdc].\n", pipeline_info->pipeline_index);
		ret = create_gdc_vnode(pipe_contex, pipeline_info->gdc_bin_file, input_width, input_height);
		ERR_CON_EQ(ret, 0);
	}else{
		printf("	[%d] not use [gdc].\n", pipeline_info->pipeline_index);
	}
	...
	if(scene_type == PIPELINE_SCENE_ISP_BYPASS) {
		...
		printf("[Error] unsupport scene type %d\n", scene_type);
		return -1;
	}
...
void *encode_pipe_data(void *context)
{
	...
	//gdc output
	hbn_vnode_handle_t gdc_node_handle = pipe_contex->gdc_node_handle;
	...
	hb_mem_graphic_buf_t* out_img_for_codec = NULL;
	...
	while (running) {
		...
		if(is_gdc_enable) {
			ret = hbn_vnode_getframe(gdc_node_handle, 0, 1000, &out_img);
			if(ret != 0) {
				printf("hbn_vnode_getframe gdc_node_handle failed.\n");
				break;
			}
			out_img_for_codec = &out_img.buffer;
		}else{
			ret = hbn_vnode_getframe_group(pym_node_handle, 0, 1000, &out_img_group);
			if(ret != 0) {
				printf("hbn_vnode_getframe_group failed.\n");
				break;
			}

			out_img_for_codec = &out_img_group.buf_group.graph_group[pipeline_info->pym_select_chn];
		}
		...
		if (verbose_flag) {
			...
			cycle_time_ms = (end_ts.tv_sec - start_ts.tv_sec) * 1000.0 +
							(end_ts.tv_nsec - start_ts.tv_nsec) / 1000000.0;
			printf("[Encode Pipe] Cycle %d time: %.2f ms\n", count, cycle_time_ms);
		}
...
int main(int argc, char** argv) {
	...
	for (index = 0; index < total_pipeline_num; index++) {
		...
		ret = create_encodec(&g_pipeline_info[index], &g_pipeline_info[index].media_context);
		if(ret != 0) {
			return -1;
		}
	}
...
static gdc_list_info_t g_gdc_list_info[] = {
	{
		.sensor_name = "imx219",
		.gdc_file_name = "../../vp_sensors/gdc_bin/imx219_gdc.bin",
		.width = 1920,
		.height = 1080,
	}
};
...
const char * get_gdc_bin_file_path(const char *sensor_name, int width, int height) {
	...
	for(int i = 0; i < sizeof(g_gdc_list_info)/sizeof(gdc_list_info_t); i++) {
		...
		if((ret == 0) && (g_gdc_list_info[i].width == width) && (g_gdc_list_info[i].height == height)) {
			gdc_file = g_gdc_list_info[i].gdc_file_name;
			break;
		}
	}
	if ((gdc_file != NULL) && access(gdc_file, F_OK) != 0) {
		printf("not found gdc file %s, so return null.", gdc_file);
		return NULL;
	}
	return gdc_file;
}
